...
├── results/              # experiment outputs as JSON (committed)
...
`courselib` is vendored in the repo, so it needs no separate install; just run
the scripts from the project root so it is importable.
...
spam), so nothing is downloaded. Note: the file is latin-1 encoded, so a UTF-8
mirror turns `£100` into `Â£100` and shifts the character kernels.
